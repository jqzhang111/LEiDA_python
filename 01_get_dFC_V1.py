...
def V1(dFC):
    ...
    for i in range(dFC.shape[2]):
        lamda = np.linalg.eig(dFC[:, :, i])
        index = np.argmax(lamda[0])
        vec = lamda[1][:, index]
        V[i, :] = np.transpose((np.real(vec)))
    return V
...
if __name__ == '__main__':
    ...
    for sub in mdd_file:
        ...
        path1 = '/share/home/zhangjiaqi/2022Project/HOPF/02_LEiDA_Empircal/dFC/MDD/'+sub[:7]
        os.makedirs(path1)
        for j in range(dfc.shape[2]):
            np.savetxt(path1+'/'+str(j)+'.txt', dfc[:, :, j], delimiter=' ')
        path2 = '/share/home/zhangjiaqi/2022Project/HOPF/02_LEiDA_Empircal/V1/MDD/'
    ...
    for sub in hc_file:
        ...
        v1 = V1(dfc)
        # store dFC and V1 for per subject
        path1 = '/share/home/zhangjiaqi/2022Project/HOPF/02_LEiDA_Empircal/dFC/HC/'+sub[:10]
        os.makedirs(path1)
        for j in range(dfc.shape[2]):
            np.savetxt(path1+'/'+str(j)+'.txt', dfc[:, :, j], delimiter=' ')
        path2 = '/share/home/zhangjiaqi/2022Project/HOPF/02_LEiDA_Empircal/V1/HC/'
